...
BRIDGE = Path(__file__).resolve().parents[1] / "modules" / "tool-computer-use" / "amplifier_module_tool_computer_use" / "bridge.ps1"
_PS = shutil.which("powershell.exe") or "/mnt/c/Windows/System32/WindowsPowerShell/v1.0/powershell.exe"
...
def _wslpath(p: str) -> str:
    return subprocess.run(["wslpath", "-u", str(p)], capture_output=True, text=True, timeout=10).stdout.strip()
...
def act(action: str, timeout: float = 60.0, **params):
    ...
    try:
        proc = subprocess.run(
            [_PS, "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", _winpath(str(BRIDGE)), "-RequestFile", _winpath(req)],
            capture_output=True,
            text=True,
            timeout=timeout,
        )
        ...
        if not out:
            return {"ok": False, "error": f"no output (rc={proc.returncode}): {proc.stderr.strip()[:400]}"}
...
if __name__ == "__main__":
    ...
    print(json.dumps(act(sys.argv[1], **json.loads(sys.argv[2]) if len(sys.argv) > 2 else {}), indent=2)[:2000])
